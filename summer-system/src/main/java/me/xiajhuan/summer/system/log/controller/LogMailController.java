 ...
 */
@RestController
@RequestMapping("log/mail")
public class LogMailController extends BaseController {

    @Resource
    private LogMailService mainService;
     ...
     */
    @GetMapping("page")
    @RequiresPermissions("log:mail:page")
    ...
    @RequiresPermissions("log:mail:excelExport")
    @RateLimiter(0.2)
    @LogOperation(OperationConst.EXCEL_EXPORT)
    public void excelExport(LogMailDto dto, HttpServletResponse response) {
        validateMaxExport(mainService.count(dto));
        ExcelUtil.export(response, "邮件日志", "邮件日志", mainService.list(dto),
                LogMailDto.class, ErrorCode.EXCEL_EXPORT_FAILURE);
    }
